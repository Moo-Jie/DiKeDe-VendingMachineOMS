...
public class PartnerController extends BaseController
{
     ...
     */
    @PreAuthorize("@ss.hasPermi('manage:partner:edit')")
    @Log(title = "修改合作商用户密码管理", businessType = BusinessType.UPDATE)
    @PutMapping("/resetPwd/{id}")
    public AjaxResult editPwd(@PathVariable Long id)
    {
        Partner partner = new Partner();
        partner.setId(id);
        partner.setPassword(SecurityUtils.encryptPassword(DkdContants.INIT_PASSWORD));//设置初始密码为123456
        return toAjax(partnerService.updatePartner(partner));
    }
}
